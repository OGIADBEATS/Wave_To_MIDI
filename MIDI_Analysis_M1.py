...
parent_directory = '/content/drive/MyDrive/groove'
file_count = 0
file_names = defaultdict(list)  # ファイル名とそのパスを保存
...
def process_midi_file(file_path, genre, bpm):
    mid = mido.MidiFile(file_path)
    ticks_per_beat = mid.ticks_per_beat
    default_tempo = mido.bpm2tempo(bpm)
    current_tempo = default_tempo
    ...
    for note, values in note_timings.items():
        instrument = pitch_to_instrument.get(note, f'Unknown ({note})')
        for time, velocity in values:
            data.append({'File': os.path.basename(file_path), 'Genre': genre, 'BPM': bpm, 'Note': note, 'Instrument': instrument, 'Time': time, 'Velocity': velocity})
    return data
        ...
        for root, dirs, files in os.walk(drummer_path):
            for filename in files:
                if filename.endswith('.mid') or filename.endswith('.midi'):
                    ...
                    if genre is not None and bpm is not None:
                        midi_file_path = os.path.join(root, filename)
                        process_midi_file(midi_file_path, genre, bpm)
                        file_names[filename].append(midi_file_path)
                        file_count += 1
                        print(f"Finished processing file: {filename}")
                    else:
                        print(f"Skipping file (invalid BPM or genre): {filename}")
...
# 重複するファイル名を表示
duplicate_files = {name: paths for name, paths in file_names.items() if len(paths) > 1}
duplicate_count = sum(len(paths) for paths in duplicate_files.values())

print(f"Total number of processed MIDI files: {file_count}")
print(f"Total number of duplicate MIDI files: {duplicate_count}")
print("Duplicate files and their paths:")
